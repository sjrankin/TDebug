...
/// Extensions for OSColor (which is nothing more than a ultra-thin (compiler only) wrapper around either NSColor (if compiled with
/// the `FOR_MACOS` compiler flag) or UIColor (if not compiled with `FOR_MACOS`).
extension OSColor
{
    /// Create an OSColor with the lower 24-bits of the passed integer. Alpha is set to 1.0.
    ///
    ...
    convenience init(Hex: Int)
    {
        ...
        let b = (Hex) & 0xff
        #if FOR_MACOS
        self.init(_colorLiteralRed: Float(r) / 255.0, green: Float(g) / 255.0, blue: Float(b) / 255.0, alpha: 1.0)
        #else
        self.init(red: CGFloat(r) / 255.0, green: CGFloat(g) / 255.0, blue: CGFloat(b) / 255.0, alpha: 1.0)
        #endif
    }
    
    /// Create an OSColor with the passed hex string.
    ///
    /// - Note:
    ...
    convenience init?(HexString: String)
    {
        ...
        if Working.count == 8
        {
            ...
            #if FOR_MACOS
            self.init(_colorLiteralRed: Float(FRed), green: Float(FGreen), blue: Float(FBlue), alpha: Float(FAlpha))
            #else
        ...
        else
        {
            ...
            #if FOR_MACOS
            self.init(_colorLiteralRed: Float(FRed), green: Float(FGreen), blue: Float(FBlue), alpha: 1.0)
            #else
    ...
    /// Return the value of the instance OSColor as a hex string.
    ///
    /// - Parameters:
    ...
    public static func MakeRandomColor(_ ColorType: RandomColorTypes = .AnyColor) -> OSColor
    {
        var RRed: Double = 0.0
        ...
        #if FOR_MACOS
        return OSColor(_colorLiteralRed: Float(RRed), green: Float(RGreen), blue: Float(RBlue), alpha: 1.0)
        #else
        return OSColor(red: CGFloat(RRed), green: CGFloat(RGreen), blue: CGFloat(RBlue), alpha: 1.0)
        #endif
    }
